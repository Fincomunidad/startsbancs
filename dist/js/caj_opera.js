...
var RecordDetalle = function (_React$Component13) {
    ...
    _createClass(RecordDetalle, [{
        ...
        value: function render() {
            ...
            var mora = numeral(this.state.mora).format('0,0.00');
            var iva = numeral(this.state.iva).format('0,0.00');
            var moraiva = numeral(this.state.moraiva).format('0,0.00');
            ...
            */
            if (esquema != 'ban.') {
                ...
                // Se va agregar cuando se valide moratorios 
                //MORATORIOS 
                /*
                            moratd = <td><input className="table-input" type="text" readOnly="readOnly" id="mora[]" name="mora[]" value={mora} /> </td>
                */
                ...
                /*
                            moraivatd = <td><input className="table-input" type="text" readOnly="readOnly" id="moraiva[]" name="moraiva[]" value={moraiva} /> </td>
                */
...
var Table = function (_React$Component14) {
    ...
    _createClass(Table, [{
        ...
        value: function render() {
            ...
            // Se va agregar cuando se valide moratorios
            //MORATORIOS
            //        let encabezado = ['','No. Socia', 'ahorroc', 'ahorrov', 'id', 'No', 'Nombre', 'Pagaré', 'No.Pagos', 'Capital', 'Interes', 'Moratorios','IVA','Mora IVA', 'SubTotal', 'Ahorro comprometido', 'Ahorro', 'Total', 'Ajuste'];
            var encabezado = ['', 'No. Socia', 'ahorroc', 'ahorrov', 'id', 'No', 'Nombre', 'Pagaré', 'No.Pagos', 'Capital', 'Interes', 'IVA', 'Ahorro comprometido', 'Ahorro', 'Total', 'Ajuste'];
    ...
    _createClass(TableGlob, [{
        ...
        value: function GeneraTotales() {
            ...
            if (fecha != '' && dias != 0 || dias != 0 && fecha != "") {
                ...
                /*
                            let nWeek = moment(fecha, 'DD/MM/YYYY').add(dias, 'day').format('e');
                            if (nWeek == 6) {
            ...
            if (importe != 0 && tasa != 0) {
                ...
                /*
                            let interes = numeral(((parseFloat(importe.replace(',', '')) * dias) * (tasa / 100)) / 360).format('0,0.00');
                            let sumTotal = numeral(parseFloat(importe.replace(',', '')) + parseFloat(interes.replace(',', ''))).format('0.00');
        ...
        value: function handleInversion(e, numero) {
            ...
            ajax(object).then(function resolve(response) {
                ...
                forma.state.catFestivos = forma.props.catFestivos;
            }, function reject(reason) {
                var response = validaError(reason);
                forma.setState({
                    csrf: response.newtoken,
        ...
        value: function handleInputChange(e) {
            ...
            if (name == "importe" || name == "movimiento") {
                ...
                if (fecha != '' && _dias != 0 || _dias != 0 && fecha != "") {
                    ...
                    this.setState({ fechafin: fechahab });
                    this.calculaInteres(_importe2, diasfinal, _tasa2);
                ...
                if (_importe2 != 0 && _tasa2 != 0) {
                    this.calculaInteres(_importe2, _dias, _tasa2);
                    /*
                                    let interes = numeral(((parseFloat(importe.replace(',', '')) * dias) * (tasa / 100)) / 360).format('0,0.00');
                                    let sumTotal = numeral(parseFloat(importe.replace(',', '')) + parseFloat(interes.replace(',', ''))).format('0.00');
                                    this.setState({ interes: interes, total: sumTotal });
                ...
                if (parseFloat(value) > parseFloat(intNor) + parseFloat(intMor)) {
                    ...
                    if (parseFloat(value) >= parseFloat(intMor) && parseFloat(intMor) > 0.00) {
                    ...
                    } else if (parseFloat(value) < parseFloat(intMor) && parseFloat(intMor) > 0.00) {
                ...
                this.setState({
                    ...
                    pago_talafecha: numeral(parseFloat(capitalactual) + parseFloat(capital_porvencer) + parseFloat(saldoIntMor) + parseFloat(saldoIntNor) + parseFloat(iva_pagoc)).format('0,0.00')
        ...
        value: function handleSubmitPago() {
            ...
            if (numeral(this.state.importepagar).format('0.00') == 0.00) {
                ...
                if (parseFloat(numeral(this.state.importepagar).format('0.00')) > parseFloat(numeral(this.state.pago_talafecha).format('0.00')) + 1) {
                    ...
                    console.log(parseFloat(numeral(this.state.importepagar).format('0.00')));
                    console.log(parseFloat(numeral(this.state.pago_talafecha).format('0.00')));

                    valida = false;
                    message = 'importe de pago es mayor al permitido';
                }
            }
...
var Captura = function (_React$Component22) {
    ...
    function Captura(props) {
        ...
        _this29.state = (_this29$state = {
            ...
            hora: '',
            vale: ''
        }, _defineProperty(_this29$state, 'semana', ''), _defineProperty(_this29$state, 'caja', ''), _defineProperty(_this29$state, 'idautoriza', 0), _defineProperty(_this29$state, 'catAutoriza', []), _defineProperty(_this29$state, 'totalautoriza', 0), _defineProperty(_this29$state, 'deposito', 0), _defineProperty(_this29$state, 'status', ''), _defineProperty(_this29$state, 'catFestivos', []), _this29$state);
        ...
        value: function handleInputChange(event) {
            ...
            if (name === "idcolmena") {
                ...
                if (datPag != "") {
                    ...
                    var importeletras = covertirNumLetras(datPag[0].monto);
                    this.setState({ importeletra: importeletras });
                } else {
                    this.setState({
                        colmena: '', grupo: '',
                ...
                if (_datPag != "") {
                    this.setState({
                        ...
                        fecha_aprov_s: _datPag[0].fecha_aprov, fecha_disper_s: _datPag[0].fecha_dispersa, monto_s: numeral(_datPag[0].seguro).format('0,0.00'), esquema: _datPag[0].esquema
                    });
                } else {
                    this.setState({
                        colmena_s: '', grupo_s: '',
                ...
                $('.mini.modal').modal({
                    ...
                    onApprove: function onApprove() {
                        ...
                        ajax(object).then(function resolve(response) {
                            ...
                            if (nomov != 0) {
                                _forma12.PrintMov(nomov);
                            }
                        }, function reject(reason) {
                            var response = validaError(reason);

                            if (response.todo.code == "200") {
                                var _idpag = _forma12.state.idpagdis;
                                var catNew = _forma12.state.catPagDis.filter(function (pag) {
                                ...
                                React.createElement(
                                    ...
                                    React.createElement(
                                        ...
                                        React.createElement(
                                            'div',
                                            { className: 'row' },
                                            React.createElement(TableGlob, { name: 'inversiones', csrf: this.state.csrf, catFestivos: this.state.catFestivos, catPlazos: this.state.catPlazos, catInteres: this.state.catInteres, datos: this.state.catInver, enca: ['Número', 'Fecha Apertura', 'Fecha vencimiento', 'Dias x vencer', 'Acciones'], onChange: this.handleChangeOpc, onClick: this.handleClickUpdate })
                                        )
                                    ),
                                    React.createElement('br', null)
                                )
                            ...
                            React.createElement(
                                ...
                                React.createElement(
                                    ...
                                    React.createElement(
                                        'div',
                                        { className: 'fields' },
                                        React.createElement(SelectOption, { name: 'movimiento', id: 'movimiento', cols: 'three wide', label: 'Tipo de Movimiento', valor: this.state.movimiento, valores: [{ name: "Deposito", value: "D" }, { name: "Retiro", value: "R" }], onChange: this.handleInputChange }),
                                        ...
                                        React.createElement(InputField, { id: 'importeletra_s', cols: 'seven wide', label: 'Importe en Letra', readOnly: 'readOnly', valor: this.state.importeletra_s, onChange: this.handleInputChange.bind(this) }),
