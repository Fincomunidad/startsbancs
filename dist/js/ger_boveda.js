...
var Captura = function (_React$Component8) {
    ...
    _createClass(Captura, [{
        ...
        value: function componentWillMount() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    this.setState({ catBoveda: catboveda, catDenoIni: response.denomina
                    });
                }.bind(this),
        ...
        value: function handleInputChange(event) {
            ...
            if (name == "idclave" && value != '0') {
                ...
                ajax(object).then(function resolve(response) {
                    forma.setState({ ocultacierre: true, fecierreant: '', dropdownDisabled: true });
                    if (response.result == false || response.result == []) {
        ...
        value: function handleButton(e, opc) {
            if (e == 0) {
                this.setState({ idclave: '0', boton: "Apertura", idmov: 0, fecierreant: '', ocultar: true, ocultacierre: true, catMov: [], dropdownDisabled: false });
        ...
        value: function findMov() {
            if (this.state.idclave != "" && this.state.idclave != "0") {
                var forma = this;
                var idmov = this.state.idmov;
                ...
                var object = {
                    url: base_url + ("api/CarteraV1/getboveda_mov/" + idmov + "/" + fechaconsulta + "/" + idclave),
                    type: 'GET',
                    dataType: 'json'
                };
                ajax(object).then(function resolve(response) {
            ...
            if (this.state.idmovdet != 0) {
                ...
                if (opc == 0 && $('#data-name').text().substring(0, 19) === 'Movimientos del dia') {
                    ...
                    if ($('#data-name').text().substring(0, 17) === 'Detalle de boveda') {
                        ...
                        surl = 'cajaopera/' + idclave + '/' + _fechaconsulta2 + '/' + fechafin;
                    }
        ...
        value: function handleSubmitOpen(e) {
            e.preventDefault();
            if (this.state.idclave == '0') {
                return;
            }
            ...
            $('.ui.form.formopen').form({
                ...
                fields: {
                    idclave: {
                        ...
                        rules: [{
                            type: 'empty',
                            prompt: 'Número de Cuenta incorrecto '
                        }]
                    }
                }
            });
        ...
        key: "handleSubmitMov",
        value: function handleSubmitMov(e) {
            e.preventDefault();
            var rulesMatchImporte = void 0;
            if (esquema != 'ban.' && esquema != 'imp.') {
                rulesMatchImporte = [{
            ...
            } else if (this.state.importe == 0 && this.state.totalxpagar == 0 && esquema != 'ban.' && esquema != 'imp.') {
                valida = false;
