...
class Ion_auth_model extends CI_Model
{
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			if ($successfully_changed_password_in_db)
			{
				...
				$this->db->update($this->tables['users'], $data, array($this->identity_column => $identity));
				$this->session->set_userdata('change_password', false);
	 ...
	 * @author Mathew
	 **/
	public function register($identity, $password, $email, $additional_data = array(), $groups = array(), $sucursal = array(), $sucursalban = array(), $sucursalama = array())
	{	
...
//		print_r($this->_filter_data($this->tables['users'], $additional_data));
...
//		die();
		...
		$this->db->insert('security.users_profiles', array('user_id' => $id, 'profile_id' => 1));
		...
		if (!empty($sucursal))
		{
			...
			{
				$this->add_to_sucursal($sucursales, $id,'fin.');
			}
		}


		if (!empty($sucursalban))
		{
			foreach ($sucursalban as $sucursales)
			{
				$this->add_to_sucursal($sucursales, $id,'ban.');
			}
		}



		if (!empty($sucursalama))
		{
			foreach ($sucursalama as $sucursales)
			{
				$this->add_to_sucursal($sucursales, $id,'ama.');
			}
		}


		if (!empty($sucursalimp))
		{
			foreach ($sucursalimp as $sucursales)
			{
				$this->add_to_sucursal($sucursales, $id,'imp.');
			}
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, lock, changepwd')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	 ...
	 * Adicion al proyecto
	 **/
	public function loginValidate($identity, $password, $option, $vs)
	{
		if (empty($identity) || empty($password || empty($option) ||  empty($vs)))
		{
			$this->set_error('login_unsuccessful');
			return FALSE;
		}
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($vs == FALSE) {
				if ($user->id != $this->ion_auth->user()->row()->id){
					$this->set_message('login_different');
					return FALSE;
				}
			}else if ($vs == TRUE) {
				if ($user->id == $this->ion_auth->user()->row()->id){
					$this->set_message('login_equal');
					return FALSE;
				}				
			}
			
			$query2 = $this->db->select('password, salt')
							->where('id', $user->id)
							->limit(1)
							->order_by('id', 'desc')
							->get($this->tables['users']);
			$hash_password_db = $query2->row();
			if ($query2->num_rows() == 1){
				...
				if ($this->hash_method == 'bcrypt')
				{
					if ($this->bcrypt->verify($password,$hash_password_db->password))
					{
						$idgrupo = $this->validaGrupo();
						$iduser = $user->id;
						$query3 = $this->accessOption($idgrupo, $iduser, $option);
						if ($query3){
							$entrar = TRUE;
						}else{
							$entrar = FALSE;
		...
		if ($entrar == TRUE) {
			$this->set_message('login_validate');
			return TRUE;
		}else{
			$this->set_error('login_unvalidate');				
			return FALSE;
	...
	public function login_attempts_exceeded($identity) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			$max_attempts = $this->config->item('maximum_login_attempts', 'ion_auth');
			if ($max_attempts > 0) {
				$attempts = $this->get_attempts_num($identity);
				if (strstr($this->errors(), $this->lang->line('login_timeout')))
				{
					return "";
				}
				return "Intento ".$attempts. " de ". $max_attempts;
			}
		}
		return "";
	}
	...
	public function add_to_sucursal($sucursal_ids, $user_id=false, $esquema)
	{
		// if no id was passed use the current users id
		$user_id || $user_id = $this->session->userdata('user_id');
		if(!is_array($sucursal_ids))
		{
			$sucursal_ids = array($sucursal_ids);
		}

		$return = 0;

		// Then insert each into the database
		foreach ($sucursal_ids as $sucursal_id)
		{

			if ($this->db->insert('security.users_sucursales', array('sistema' => $esquema, 'sucursal_id' => $sucursal_id, 'user_id' => (float)$user_id)))
			{
				// Return the number of groups added
				$return += 1;
			}
	...
	public function remove_from_sucursal($sucursal_ids=false, $user_id=false, $esquema)
	{
		// user id is required
		if(empty($user_id))
		{
		...
		if( ! empty($sucursal_ids))
		{
			...
			foreach($sucursal_ids as $sucursal_id)
			{
				$this->db->delete('security.users_sucursales', array('sucursal_id' => (float)$sucursal_id, 'user_id' => (float)$user_id));
			}
		...
		{
			if ($return = $this->db->delete('security.users_sucursales', array('sistema' => $esquema, 'user_id' => (float)$user_id))) {
			}
		}
		return $return;
	}
...
//		$columns = $this->db->list_fields($table);
		$columns = $this->db->query('SELECT * FROM '.$table)->list_fields(); 
	...
	*¨ por la integración de tablas
	* 
	*
	*/

	function options_menu($idgrupo, $keys=array('')){
		//$keys = array('');
		
		if ($this->session->userdata('esquema') != 'ama.' && $this->session->userdata('esquema') != 'imp.'){
			//$keys = array('1014');
			array_push($keys, '1014');
		}
		if ($this->session->userdata('esquema') != 'ban.'){
			array_push($keys, '1015');
		}
		...
		$query = $this->db->select('options.group_id, options.key, options.name, options.level, options.option, options.link, options.icon')
				->from($this->tables['options'])
				->join('security.users_groups', 'security.users_groups.group_id = security.options.group_id')
				->where_in('security.options.group_id', $idgrupo)
				->where('security.users_groups.user_id', $id)				
				->where('security.options.status', '1')
				->where_not_in('security.options.key',$keys)
    ...
    function accessOption($idgrupo, $id, $key){
		$query = $this->db->select('options.group_id, options.key, options.name, options.level, options.option, options.link, options.icon')
				...
				->where('security.options.key', $key)
				->group_by(array('options.group_id', 'options.key', 'options.name', 'options.level', 'options.option', 'options.link', 'options.icon'))
				->order_by('options.key')
				->get();
		if ($query->num_rows() > 0 ){
	...
	function validaGrupo(){
		$opc = $this->session->userdata('opcion');
		if ($opc == "contabil"){
			return "1";
		} else if ($opc =="cartera"){
			return "2";
