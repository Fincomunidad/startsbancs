...
*/
	...
	public function add_crediton_post() {
		...
		$adicionDatos = array('idproducto' => 2, 'idejecutivo' => 1);
...
*/
	...
	public function get_solicitud_credito_get() {
		$idcredito = $this->uri->segment(4);
            ...
            $fields = array("idcredito", "fecha", "idpagare", "acreditadoid", "direccion", "edocivil", "edocivil_nombre", "idactividad", "actividad_nombre", "idcolmena", "nomcolmena", "idgrupo", "nomgrupo", "nivel", "monto", "proy_nombre", "proy_descri", "proy_lugar", "proy_observa", "idchecklist", "fecha_aprov", "usuario_aprov", "fecha_pago", "periodo", "num_pagos","tasa", "fecha_pago2");
		$where = array("idcredito"=>$idcredito);
	...
	public function find_acreditados_get(){
            ...
            $query = "SELECT c.idsucursal, c.idcredito, c.idgrupo, c.nivel, c.idpagare, upper(a.acreditado) as nombre, a.rfc, a.idacreditado, a.acreditadoid
                        FROM fin.creditos as c JOIN get_acreditados as a ON c.idacreditado=a.acreditadoid WHERE upper(a.acreditado) like '%".$nombre."%' ORDER BY fecha_aprov limit 8";
		$acredita = $this->base->querySelect($query, FALSE);
