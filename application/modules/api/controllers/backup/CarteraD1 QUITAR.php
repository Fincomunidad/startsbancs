...
*/
	...
	public function find_acreditados_get(){
            ...
            $query = "SELECT c.idsucursal, c.idcredito, c.idgrupo, c.nivel, c.idpagare, upper(a.acreditado) as nombre, a.rfc, a.idacreditado, a.acreditadoid
                        FROM ".$this->esquema."creditos as c JOIN get_acreditados as a ON c.idacreditado=a.acreditadoid WHERE idproducto<>10 and upper(a.acreditado) like '%".$nombre."%' ORDER BY fecha_aprov limit 8";
		$acredita = $this->base->querySelect($query, FALSE);
	...
	public function find_acreditadosInd_get(){
            $nombre = $_GET["q"];
            $query = "SELECT c.idsucursal, c.idcredito, c.idgrupo, c.nivel, c.idpagare, upper(a.acreditado) as nombre, a.rfc, a.idacreditado, a.acreditadoid
                        FROM ".$this->esquema."creditos as c JOIN get_acreditados as a ON c.idacreditado=a.acreditadoid WHERE c.idproducto=10 and upper(a.acreditado) like '%".$nombre."%' ORDER BY fecha_aprov limit 8";
		$acredita = $this->base->querySelect($query, FALSE);
		$this->validaCode($acredita);
	}
	...
	public function colmena_query_get(){
      ...
      public function get_colmena_id_get(){
		$numero = $this->uri->segment(4);
            //$where = array('acreditadoid' => $valor);
		$colmena = $this->base->querySelect("SELECT idcolmena, idsucursal, idmunicipio, idcolonia, numero, nombre, dia, idpromotor, fecha_apertura, empresa, direccion, extract(hour from horainicio) as horas, extract(minute from horainicio) as minutos 
	...
	*/
	public function catcolonia_get(){
      ...
      public function getcreditos_sin_entregar_get(){
            $numero = $this->uri->segment(4);
		$catCreditos = $this->base->querySelect("SELECT idcredito as value, (acreditado || ' - ' || idpagare)::varchar as name 
                  FROM (
