		...
		if ($amortiza) {
			...
			foreach($amortiza as $key => $value){
				if ($amortiza[$key]['fecha_entrega'] === null || $amortiza[$key]['fecha_entrega'] === '' ) {
					...
					$data = $this->base->querySelect("select idcredito, numero, capital, interes, iva, garantia, total, ajuste, asistencia, incidencia, ahorro_vol,  to_char(fecha_pago_col,'DD/MM/YYYY HH:MI:SS') as fecha_pago_col, extract(week from a.fecha_pago_col::date) as iweek,
