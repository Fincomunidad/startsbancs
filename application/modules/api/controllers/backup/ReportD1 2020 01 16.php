...
//		}
	...
	*/
	...
	public function tableEdoCuenta($title, $data) {
		...
		foreach($data as $key => $value) {
			...
			if ($value['numero']<>-99){
				...
				if ($value['fecha_pago'] === null ){
					...
					$html.='  <td align="right">'.number_format($value['c_iva'], 2, '.', ',').'</td>';
					$html.='  <td> </td>';
				}else{
		...
		{
			$title2 = array("Capital requerido", "Interes", "IVA", "Pago total amortización");		
			$html2='<br><br>';
			...
			$title3 = array("Capital a liquidar", "Interes", "IVA", "Pago total para liquidar");
			$html3='<br><br>';
	...
	public function pdf_edo_cuenta_get(){
		...
		$amor = $this->base->querySelect("SELECT numero, fecha_vence as fecha, p_fecha as fecha_pago, dias, p_capital as pag_capital, p_interes as interes, p_iva as iva, p_pago_total as total_pagado,
			capital_saldo as saldo_capital, coalesce(p_mora,0) as int_mora, capital_pagado, capital_requerido, c_interes_acumula, c_mora, c_iva  FROM ama.ftr_pago_individual(".$idcredito.", current_date)", TRUE);
		//print_r($data);
		//die();
		$title = array("Pago","Vencimiento", "Pago", "Dias", "Capital pagado", "Interes", "IVA", "Pago total", "Saldo capital", "Int. mora", );
		$tabla = '';
...
*/
	...
	public function pdf_creditos_activos_get(){
		...
		if ($tipo==="1"){
			...
			$title = array("Empresa","Núm.", "Colmena", "Grupo", "Acreditada", "Suc.", "Credito", "Pagare", "Empresa");
			$query = "SELECT sistema, col_numero, col_nombre, grupo_nombre, nombre, idsucursal, idcredito,  idpagare, empresa from public.v_creditos_vigentes ORDER BY sistema, empresa, idsucursal, col_numero, grupo_nombre";
		}
