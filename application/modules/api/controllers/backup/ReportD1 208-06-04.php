...
//		}
	...
	public function pdf_solicitud_credito_get(){
		...
		$fields = array("nombre", "fin.get_cargo_colmena(acreditadoid) as cargo");
		$where = array("acreditadoid"=>$cred['idaval1']);
		$aval = $this->base->selectRecord("fin.get_acreditado_solicitud", $fields, "", $where, "","", "", "", "","", TRUE);
		$aval = $aval[0];
	...
	*/
	...
	public function pdf_contrato_get(){
		...
		$fields = array("nombre", "fin.get_cargo_colmena(acreditadoid) as cargo");
		$where = array("acreditadoid"=>$cred['idaval1']);
		$aval1 = $this->base->selectRecord("fin.get_acreditado_solicitud", $fields, "", $where, "","", "", "", "","", TRUE);
		if ($aval1) {
		...
		$aval2 = $this->base->selectRecord("fin.get_acreditado_solicitud", $fields, "", $where, "","", "", "", "","", TRUE);
		if ($aval2) {
		...
		<br>
		<p>Primera.- Importe. La ACREDITANTE entrega al ACREDITADO en la fecha de firma del presente contrato la cantidad de $'.$monto.'– '.strtoupper($monto_letra).' El acreditado se obliga a pagar el crédito de conformidad con los pagos fijados en la cláusula tercera.</p>
		...
		<p>Este contrato se firma el  '.date_format($fecha,'d/m/Y').', en ZIMATLAN DE ALVAREZ, OAXACA</p>
		<br>
		<br>
		<br>
		<table style="width:100%" border=0>
	...
	public function pdf_ahorro_get(){
		...
		$fields = array("idcredito", "idacreditado", "idgrupo", "idsucursal", "fecha", "proy_nombre", "proy_descri", "proy_lugar", "idproducto", "nivel", "monto", "idpagare", "idchecklist", "idejecutivo", "fecha_aprov", "usuario_aprov", "fecha_mov", "usuario", "idpersona", "nombre", "tipo", "edocivil", "sexo", "idactividad", "direccion", "idcolmena", "fecha_entrega_col");
		$where = array("idcredito"=>$idcredito);
		$cred = $this->base->selectRecord("fin.get_solicitud_credito", $fields, "", $where, "","", "", "", "","", TRUE);
		$cred = $cred[0];
		$nombre = $cred['nombre'];
...
*/
        ...
        $html.='<br> 
			<table style="width:100%"  border="0">
				...
				<tr>
					...
					<td></td>
					<td style="border-top: 1px solid" align="center"  width="25%">'.$aval['nombre'].'</td>
					<td></td>
				</tr>		
			</table>';				
	...
	public function pdf_tabla_amortizacion_nueva_get(){
		...
		$fields = array("numero", "fecha_vence", "saldo_capital","saldo_capital2","capital", "capital_pag", "interes_pag", "iva_pag", "aportesol", "garantia", "total" );
		$where = array("idcredito"=>$idcredito);
		$order_by = array(array('campo'=> 'numero', 'direccion'=>	'asc'));
		$amor = $this->base->selectRecord($this->session->userdata('esquema')."amortizaciones", $fields, "", $where, "","", "", $order_by, "","", TRUE);
		$title = array("Pago","Vencimiento","Saldo Capital", "Interes", "IVA", "Capital", "Cuota");
	...
	public function pdf_retgarantia_acreditado_get(){
		...
		$cred = $this->base->selectRecord("fin.get_acreditado_grupo", $fields, "", $where, "","", "", "", "","", TRUE);
		$cred = $cred[0];
