...
//		$header.= '<p align="center">MATRIZ - ZIMATLAN</p>';
		...
		$html.='<table style="font-size:13px; width:100%" border="0" class="100p">
			...
			<tr>
				<th class="seccion-left">
					<br>
					C. ADRIANA COINTA GERONIMO DIAZ
				</th>
				<th></th>
				<th class="seccion-right">
					<br>
...
//		$miFecha = date_create($mov['fecha']);
	...
	public function solcreditopdf_get(){
		...
		$data =array('Reg1'=>  array('enca' => array('Primer Nombre', '','Segundo Nombre','','Apellido Paterno','','Apellido Materno'),
					...
					'datos' => array($sol['aliaspf'], '',  $sol['fecha_nac'],'', $sol['sexoc'])
						),
		...
		$html = $header.'
			...
			<h4 class="encatitulo">DATOS PERSONALES</h4>';
		$html.=$this->tableCreate($data);
		$html.='<br>
			<h4 class="encatitulo">ACTIVIDAD ECONÓMICA</h4>';
		$html.=$this->tableCreate($data1);
		$html.='<br>
			<h4 class="encatitulo">DOMICILIO DEL SOLICITANTE</h4>';
		$html.=$this->tableCreate($data2);
		$html.='<br>
			<h4 class="encatitulo">BENEFICIARIO</h4>';
		$html.=$this->tableCreate($data3);

		$html.='<br>
		<p align="center" style="padding-left: 10px; padding-right: 10px; border: 1px solid rgba(34,36,38,.15)!important;">Recibí la información del funcionamiento, organización, reglas y políticas de operación de la institución,
		solicito voluntariamente mi ingreso como socio y me comprometo a cumplir las obligaciones que emanen de tal calidad. 
		</p>
		<p>NOMBE Y FIRMA DEL SOLICITANTE</p>
		<p>'. $sol['nombre1'].($sol['nombre2']!=''?' '.$sol['nombre2']:'').' '.$sol['apaterno'].' '.$sol['amaterno'].'</p>
		<p>LUGAR Y FECHA DE SOLICITUD</p>
		<p>'.$emp["domicilio"].', '.$emp["colonia"].', '.$emp["municipio"].', Oaxaca'.' a '.$fechaletras.'</p>
		<p>SE RESERVA EL DERECHO DE ADMISIÓN CONFORME A LOS ESTATUS VIGENTES APROBADOS POR EL CONSEJO DE ADMINISTRACIÓN</p>
...
//		}
	...
	function resumenPolDia($resumen, $valor, $global){
		...
		foreach($filtra as $key => $value2) {
				...
				if ($value2['nombre'] == 'PAGO DE CREDITO') {
	...
	public function edocta_get(){
		...
		$fields = array("idcredito", "idacreditado", "idgrupo", "idsucursal", "fecha", "fecha_entrega_col","to_char(fecha_entrega_col,'DD/MM/YYYY') as fecha_entrega_col1","proy_nombre", "proy_descri", "proy_lugar", "idproducto", "nivel", "monto", "idpagare", "idchecklist", "idejecutivo", "fecha_aprov", "usuario_aprov", "fecha_mov", "usuario", "idpersona", "nombre", "tipo", "edocivil", "sexo", "idactividad", "direccion", "idcolmena");
...
*/		
	...
	public function aportasoc_get(){
		...
		$sSucursal = $suc =="99"?"CONSOLIDADO":'SUCURSAL '.strtoupper($this->session->userdata('nomsucursal'));
		$empresal = $this->getEmpresa();
		$tiempo =  date("H:m");
		$user = $this->ion_auth->user()->row()->first_name.' '.$this->ion_auth->user()->row()->last_name;
 ...
 */
 	public function asisglobal_get(){
		...
		$data = $this->base->querySelect("select * FROM col.get_asistencia_global(".$idpromotor.",".$anio.",".$mes.",'".$esquema."')", true);
		...
		';
			...
			foreach($data as $key => $value) {
				$html.='  <tr>';
				$html.='  <td style="text-align:center; width:10%">'.$value['idcolmena'].'</td>';
				$html.='  <td style="width:35%">'.$value['nombre'].'</td>';
				$html.='  <td>'.$value['mespasado'].'</td>';
				$html.='  <td>'.$value['integradas'].'</td>';
				$html.='  <td>'.$value['nuevas'].'</td>';
				$html.='  <td>'.$value['reingresos'].'</td>';
				$html.='  <td>'.$value['inactivas'].'</td>';
				$html.='  <td>'.$value['renuncias'].'</td>';
				$html.='  <td>'.$value['total'].'</td>';
				$html.='  <td>'.$value['porcapacitar'].'</td>';
				$html.='  <td>'.$value['grupomespasado'].'</td>';
				$html.='  <td>'.$value['integrados'].'</td>';
				$html.='  <td>'.$value['cerrados'].'</td>';
				$html.='  <td>'.$value['totalgrupo'].'</td>';
				$html.='  <td>'.$value['col_actual'].'</td>';
				$html.='  <td>'.$value['col_nuevas'].'</td>';
				$html.='  <td>'.$value['col_cerradas'].'</td>';
				$html.='  <td>'.$value['col_total'].'</td>';
				$html.='  </tr>';
				...
				$html.='  <tr>';
				$html.='  <td></td>';
				$html.='  <td style="width:35%">TOTALES</td>';
				$html.='  <td>'.$t1.'</td>';
				$html.='  <td>'.$t2.'</td>';
				$html.='  <td>'.$t3.'</td>';
				$html.='  <td>'.$t4.'</td>';
