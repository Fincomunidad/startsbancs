	...
	public function add_cuentaho_post() {
		...
		$valores['numero_cuenta'] = $valores['numero_cuentanew'];
			...
			if ($this->form_validation->run('ahorros_put') == TRUE) {
				$updatetrans = $this->ahorro->transaction($datos);
...
*/
	public function solcreditogen_post() {
		...
		$this->insertData('persona', $valores, 'solingresogen_put');
	}
	...
	public function solcreditogen_put() {
		...
		$fecha = explode("/", $valores['fecha_nac']); 
		$valores['fecha_nac'] =  $fecha[2] .'-'.$fecha[1].'-'.$fecha[0] ;
		$this->updateData('persona', $valores, 'solingresogen_put', $where, false);
	}
...
//		$datos['fechaalta'] = $datos['fechaaltab'];
	...
	public function solcredito_get() {
		...
		$ben = $this->base->selectRecord('public.personas', 'public.personas.*, public.persona_ben.idparentesco, public.persona_ben.porcentaje', $join, $where, "","", "", "", "","", TRUE);
		if ($solcre) {
				$data = array("status"=>"OK",
				"code" => 200,
				"message"=>"Solicitud obtenida correctamente!",
	...
	public function solcreacre_get(){
		...
		$query ="select p.idsucursal, p.idpersona,  CASE WHEN p.tipo::text = 'M'::text THEN p.cia::text ELSE ((COALESCE(p.nombre1, ''::character varying)::text || 
			...
			".$this->esquema."get_colmena_grupo as c on c.idgrupo = b.idgrupo  where  CASE WHEN p.tipo::text = 'M'::text THEN p.cia::text ELSE ((COALESCE(p.nombre1, ''::character varying)::text ||
			CASE WHEN btrim(COALESCE(p.nombre2, ''::character varying)::text) <> ''::text THEN COALESCE(' '::text || p.nombre2::text, ''::character varying::text) ELSE ''::text END) ||
			CASE WHEN btrim(COALESCE(p.apaterno, ''::character varying)::text) <> ''::text THEN COALESCE(' '::text || p.apaterno::text, ''::character varying::text) ELSE ''::text END) ||
			CASE WHEN btrim(COALESCE(p.amaterno, ''::character varying)::text) <> ''::text THEN COALESCE(' '::text || p.amaterno::text, ''::character varying::text) ELSE ''::text END END like '%".$nombre."%' limit 8";
		$acredita = $this->base->querySelect($query, FALSE);
...
*/
	...
	public function findAcre_get(){
		...
		if (!is_numeric($idacreditado)){
			...
			$query = "Select a.*, b.idcredito from ".$esquema."get_acreditado_solicitud as a left join ".$esquema."creditos as b on b.idacreditado = a.acreditadoid where a.idacreditado=".$idacreditado.' limit 1';
			$acredita = $this->base->querySelect($query, FALSE);
			$this->validaCode($acredita);
		}
	}
...
public function findPolizaDiario($fecha) {
		...
		if ($mov) {
			if ($this->esquema == 'ban.'){
				$query = "select nombre, count(nomov) as numero, sum(importe) as importe, sum(importe - aportesol) as capital, sum(interes) as interes,  sum(iva) as iva  from ".$this->esquema."get_movimientosdia('".$fecha."','".$fecha."','0','".$this->session->userdata('sucursal_id')."') group by orden,nombre order by orden";
			}else {
...
//				}
	...
	public function getCreditos_get(){
		...
		if (!is_numeric($idacreditado)){
		   $this->returnCode();
			...
			$esquema = $this->esquema;       
			$query = "Select idcredito, idpagare, monto, pagos, saldo, numero_cuenta, idahorro, comprometido, idproducto from ".$esquema."get_creditos_resumen(".$idacreditado.") where fecha_liquida is null or comprometido<>0 ";
	...
	* Aportación Social
	...
	public function findAportaSoc($fecha,$suc) {
		...
		$query = "select idsucursal, idacreditado, nombre, saldo, 0 as parcial,fecha_alta, 0 as apor, localidad, 1 as ct,direccion, fecha_nac, sexo from ".$this->esquema."get_aporta_socialg('".$fecha."')".$sFiltro;
		$mov = $this->base->querySelect($query, TRUE);		
		if ($mov) {
			$respuesta = array("status"=>"OK",
				"code" => "200",
	...
	public function getAportaSocLast($iacreditado, $fecha){
		$sFiltro = " where idacreditado =".$iacreditado." and fecha::date <='".$fecha."'";
...
*/
		...
		$query = "select * from (
				select idsucursal, idacreditado,nombre, producto,fecha_alta,fecha_baja, tasa, saldo,0 as interes, saldo as capint from ".$this->esquema."get_ahorros_voluntariosg('R','".$fecha."')".$sFiltro."				
				) as a ".$orderby;
		$mov = $this->base->querySelect($query, TRUE);		
		if ($mov) {
			$respuesta = array("status"=>"OK",
...
*/
		...
		$orderby = $suc =="99"?" order by a.idsucursal, a.idacreditado":" order by a.idacreditado, a.producto";
		$query = "select * from (				
				select idsucursal, idacreditado,nombre, producto,fecha_alta,fecha_baja, tasa, saldo,interes, saldo as capint from ".$this->esquema."get_inversiones('".$fecha."')".$sFiltro."
				) as a ".$orderby;
		$mov = $this->base->querySelect($query, TRUE);		
		if ($mov) {
			$respuesta = array("status"=>"OK",
	...
	public function getInverxVencer_get(){
		...
		case when fecha_entrega is null then 'Por ingresar' else  case when estatus = true then 'Activo' else 'Aplicado' end end as estatus  from ".$esquema."get_inversiones where idsucursal ='".$this->session->userdata('sucursal_id')."' order by fechafin::date ASC";
		$inver = $this->base->querySelect($query, FALSE);
		$this->validaCode($inver);
	}
	...
	*/
	...
	public function bovedas_get(){
		$query = "Select idboveda as value, nombre as name from public.bovedas where idsucursal ='".$this->session->userdata('sucursal_id')."'";
		$boveda = $this->base->querySelect($query, TRUE);
		$query = "Select iddenomina, nombre, 0 as cantidad, 0 as total from public.cat_denomina order by iddenomina";
	...
	public function add_boveda_post() {
		...
		if (!is_numeric($idmov)){
			...
			$datos['tipo'] = 'O';
			$datos['status'] = '1';
			if ($this->esquema =='ban.' && $datos['movimiento'] == "E" && $datos['des_ori'] == "C"){
	...
	public function findCajaOpera($idsuc, $fecini, $fecfin) {

		$query = "select fecha::date,cabo,idcaja,count(cabo) as no, sum(importe) as importe from ".$this->esquema."get_movimientosdia('".$fecini."','".$fecfin."','0','".$idsuc."') where idcaja <>'' group by fecha::date, cabo,idcaja order by fecha::date,cabo,idcaja";
		$bov = $this->base->querySelect($query, TRUE);
		if ($bov) {
			$respuesta = array("status"=>"OK",
				"code" => "200",
