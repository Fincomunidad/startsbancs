                        ...
                        Where (Select count(*) from ".$this->esquema."boveda_mov"." where fecha::date ='".$fec."' and des_ori ='C' and idbanco ='".$bov['idbanco']."') <> 0 and 
							  ...
							  (
								(
									...
									and 
										(Select count(*) from ".$this->esquema."boveda_mov where fecha::date ='".$fec."' and movimiento ='E' and des_ori ='C' and idmov = ".$bov['idmov'].") -
										(Select count(*) from ".$this->esquema."boveda_mov where fecha::date ='".$fec."' and movimiento ='I' and des_ori ='C' and idmov = ".$bov['idmov'].")  >= 1
								)
								or 
								(
								    (
								  		(Select count(*) from ".$this->esquema."inversiones"." where fechafin::date ='".$fec."' and idsucursal='".$this->idsuc."' and estatus= true) <> 0 or
										(select count(*) from ".$this->esquema."creditos c where not fecha_aprov is null and not fecha_dispersa is null and fecha_entrega is null and c.idsucursal ='".$this->idsuc."' and fecha_dispersa::date ='".$fec."') <> 0 
								    ) 
								 	 and 
									  (Select count(*) from ".$this->esquema."boveda_mov where fecha::date ='".$fec."' and movimiento ='E' and des_ori ='C' and idmov = ".$bov['idmov'].") -
									  (Select count(*) from ".$this->esquema."boveda_mov where fecha::date ='".$fec."' and movimiento ='I' and des_ori ='C' and idmov = ".$bov['idmov'].")  >= 2
								)  
							  )
                              RETURNING idmovdet";
                    }else {
