...
//            $nomov = $this->base_caja->nextMovimiento();
            ...
            $sql = "insert into ".$this->esquema."inversiones "." (idacreditado, idsucursal,fecha,numero, dias, tasa, fechafin,importe, interes, isr, total, estatus, usuario, fecha_dispersa) 
                select '".$inv['idacreditado']."','".$this->idsuc."','".$fecha."','".$numero."','".$inv['dias']."','".$inv['tasa']."','".$inv['fechafin']."','".$inv['importe']."','".$inv['interes']."',false,'".$inv['total']."',true,'".$this->iduser."','".$fechadis."'
...
//            $numero = $this->base_car->nextMovimiento();
            ...
            $sql = "update ".$this->esquema."inversiones "." set fecha_entrega ='".$fechaent."'
                where idacreditado =".$inv['idacreditado']." and idsucursal ='".$this->idsuc."' and fecha::date ='".$inv['fecha']."' and numero =".$inv['numero']." and total =".$inv['total']." and fecha_entrega is null";
            $sqlUpdate[$nosql] = $sql;
            $nosql++;
    ...
    public function transreinver($pag){
		...
		for($i=0; $i<=2; $i++) {
            ...
            if (($inv['retiroccheck'] == "on" && $inv['cretiroc'] =="V") || ($inv['retiroicheck'] == "on" && $inv['cretiroi'] == "V")) {
                $ahorro = $this->base->querySelect("Select idahorro from ".$this->esquema."ahorros where idacreditado =".$inv['idacreditado']." and idsucursal ='".$this->idsuc."' and idproducto ='02' and substring(numero_cuenta,1,2) ='AV' and fecha_baja is null order by numero_cuenta ASC",true);
                if ($ahorro){
            ...
            if ($inv['retiroccheck'] == "on" && $inv['retiroicheck'] == "on") {
                ...
                $sql = "insert into ".$this->esquema."inversiones "." (idacreditado, idsucursal,fecha,numero, dias, tasa, fechafin,importe, interes, isr, total, estatus, usuario, fecha_dispersa, fecha_entrega) 
                    select '".$inv['idacreditado']."','".$this->idsuc."','".$fecha."','".$numero."','".$inv['dias']."','".$inv['tasa']."','".$inv['fechafin']."','".$inv['importe']."','".$inv['interes']."',false,'".$inv['total']."',true,'".$this->iduser."','".$fechadis."','".$fechadis."'
                    where (select count(idacreditado) from ".$this->esquema."inversiones where idacreditado =".$inv['idacreditado']." and idsucursal='".$this->idsuc."' and fecha::date ='".$inv['fecha']."') = 0
                    RETURNING idinversion";
                $sqlUpdate[$nosql] = $sql;
                $nosql++;
