    ...
    public function asistencia($pag, $edocta){
		...
		for($i=0; $i<=2; $i++) {
            ...
            foreach($pag as $key => $value){
                ...
                $sql = "insert into col.asistencia"." (acreditadoid, idgrupo, asistencia, incidencia, anio, semana, fecmov, iduser) 
                    select  '". $acreditadoid."','".$idgrupo."', '".$asistencia."', '".$incidencia."', '".$anio."', '".$semana."','".$fecha."', '".$iduser."'";
                $sqlUpdate[$nosql] = $sql;
                $nosql++;
