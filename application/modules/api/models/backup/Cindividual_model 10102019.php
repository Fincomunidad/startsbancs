    ...
    */
    public function transaccion($mov, $fecha_pago){
		...
		for($i=0; $i<=2; $i++) {
            ...
            $sql = "insert into ".$esquema."pagos"." (idcredito, fecha_pago, total_pago, condonacion, gastos, nomov, caja, idmoneda, idinstrumento) 
                        select  '". $credito['idcredito'] ."','".$fecha."','".$credito['importepagar']."','".$credito['condonacion']."','".$credito['gastos']."','".$nomov."','".$idcaja."','MXN','01'";
            $sqlUpdate[$nosql] = $sql;
            $nosql++;
