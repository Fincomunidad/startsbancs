...
//            $fecha = $inv['fecha'].' '.date("H:i:s");
            ...
            $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, idsucursal) 
            values('?','".$fecha."','D','".$instrumento. "','01','".$inv['importe']."','". $this->idsuc."')";
            $sqlUpdate[$nosql] = $sql;
            $nosql++;
    ...
    public function transreinver($pag){
		...
		for($i=0; $i<=2; $i++) {
            ...
            if ($inv['incrementocheck'] == "on" && $inv['retiroicheck'] == "false") {
                $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, idsucursal) 
                values('?','".$fecha."','D','01','01','".$inv['totali']."','". $this->idsuc."')";
                $sqlUpdate[$nosql] = $sql;
                $nosql++;

                $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, nomov, idcaja, idsucursal) 
                values('?','".$fecha."','D','01','09','".$inv['incremento']."','".$nomov."','".$this->idcaja."','". $this->idsuc."')";
                $sqlUpdate[$nosql] = $sql;
                $nosql++;
            ...
            // Incremento de capital y retiro de interes
                ...
                $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, idsucursal) 
                values('?','".$fecha."','D','01','01','".$saldo."','". $this->idsuc."')";
                $sqlUpdate[$nosql] = $sql;
                $nosql++;
                ...
                if ($inv['cretiroi'] =="E"){
                    $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, idsucursal) 
                    values('?','".$fecha."','D','01','03','".$inv['retiroi']."','". $this->idsuc."')";
                    $sqlUpdate[$nosql] = $sql;
                    $nosql++;

                    $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, nomov, idcaja, idsucursal) 
                    values('?','".$fecha."','R','01','03','".$inv['retiroi']."','".$nomov."','".$this->idcaja."','". $this->idsuc."')";
                    $sqlUpdate[$nosql] = $sql;
                    $nosql++;
            ...
            // Retiro de capital
                ...
                if ($inv['cretiroc'] =="E"){
                    ...
                    $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, idsucursal) 
                    values('?','".$fecha."','D','01','09','".$inv['retiroc']."','". $this->idsuc."')";
                    $sqlUpdate[$nosql] = $sql;
                    $nosql++;

                    $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, nomov, idcaja, idsucursal) 
                    values('?','".$fecha."','R','01','09','".$inv['retiroc']."','".$nomov."','".$this->idcaja."','". $this->idsuc."')";
                    $sqlUpdate[$nosql] = $sql;
                    $nosql++;
                    
                    $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, idsucursal) 
                    values('?','".$fecha."','D','01','01','".$inv['importe']."','". $this->idsuc."')";
                    $sqlUpdate[$nosql] = $sql;
                    $nosql++;
            ...
            if ($inv['retiroccheck'] == "on"  && $inv['retiroicheck'] == "on" ){
                if ($inv['cretiroc'] =="V" && $inv['cretiroi'] =="V" ){
                    $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, idsucursal) 
                    values('".$inv['idinversion']."','".$fecha."','R','01','09','".$inv['importei']."','". $this->idsuc."')";
                    $sqlUpdate[$nosql] = $sql;
                    $nosql++;
                }else {
                    $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, nomov, idcaja, idsucursal) 
                    values('".$inv['idinversion']."','".$fecha."','R','01','09','".$inv['importei']."','".$nomov."','".$this->idcaja."','". $this->idsuc."')";
                    $sqlUpdate[$nosql] = $sql;
                    $nosql++;
                }
                ...
                if ($inv['cretiroi'] =="E"){
                    $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, idsucursal) 
                    values('".$inv['idinversion']."','".$fecha."','D','01','03','".$inv['retiroi']."','". $this->idsuc."')";
                    $sqlUpdate[$nosql] = $sql;
                    $nosql++;

                    $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, nomov, idcaja, idsucursal) 
                    values('".$inv['idinversion']."','".$fecha."','R','01','03','".$inv['retiroi']."','".$nomov."','".$this->idcaja."','". $this->idsuc."')";
                    $sqlUpdate[$nosql] = $sql;
                    $nosql++;
                }else if ($inv['cretiroi'] =="V"){
                    $sql = "insert into ".$this->esquema."ahorros_mov"." (idahorro, fecha, idsucursal, movimiento, importe, idinstrumento, lugar, usuario, idmovimiento) 
            ...
            // Retiro de capital de la inversion original 
                ...
                $sql = "insert into ".$this->esquema."inversiones_det"." (idinversion, fecha, movimiento, idinstrumento, idmovimiento, importe, idsucursal) 
                values('".$inv['idinversion']."','".$fecha."','R','01','01','".$inv['importei']."','". $this->idsuc."')";
                $sqlUpdate[$nosql] = $sql;
                $nosql++;
            }
