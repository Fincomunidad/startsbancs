...
*/
		...
		for($i=0; $i<=2; $i++) {
            ...
            foreach($pag as $key => $value){
                ...
                if ($edocta != []) {
                    if ($edocta['instrumento']  != '') {
                        $instrumento =$edocta['instrumento'];
                    }
                }
                ...
                if ($nopagos ==1) {
                    ...
                    if ($pagare['numero'] > 0) {
                        $sql = "update ".$esquema."amortizaciones"." set fecha_pago ='".$fecha."', idcaja='".$idcaja."', nomov=".$nomov.", instrumento='".$instrumento."' where idcredito =". $pagare['idcredito']." and numero =". $pagare['numero']." and fecha_pago is null";
                        $sqlUpdate[$nosql] = $sql;
                        $nosql++;
                    ...
                    for($i=0; $i<$nopagos; $i++){
                        ...
                        $sql = "update ".$esquema."amortizaciones"." set  fecha_pago ='".$fecha."', idcaja='".$idcaja."', nomov=".$nomov.", instrumento='".$instrumento."' where idcredito =".$pagare['idcredito']." and numero =".($pagare['numero'] + $i)." and fecha_pago is null";
                        $sqlUpdate[$nosql] = $sql;
                        $nosql++;
    ...
    public function asistencia($pag, $edocta){
		...
		for($i=0; $i<=2; $i++) {
            ...
            foreach($pag as $key => $value){
                ...
                if ($opcion == 5) {
                   if (array_key_exists('inactiva', $registro )){
                        $verificacion = $registro['inactiva'];
                    }else{
