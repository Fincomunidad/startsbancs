...
*/
			...
			// actualizacion el dia 11/07/2022 se agrego la ultima condicion count( distinct (idbanco) )  >0
            $sCierreCorte = "";
            if ($existeCorteCaja == true) {
                $sCierreCorte ="and 
                (Select count(idmov) from ".$this->esquema."boveda_mov where tipo ='C' and status ='1' 
                  and idmov='".$bov['idmov']."')  =  
                (Select count( distinct (idbanco) ) from ".$this->esquema."boveda_mov where movimiento ='E' and des_ori ='C'
                 and idmov='".$bov['idmov']."') ";
            }
            //Update boveda 
            $sql = "update ".$this->esquema."boveda"." set fecfinal='".$fechacierre."', status ='0' 
                    ...
                    and (Select count(idmov) from ".$this->esquema."boveda_mov where status ='0' 
                        ...
                        (Select count(idmov) from ".$this->esquema."boveda_sal_det where idmov='".$bov['idmov']."') > 0 and 
                        (Select count(idmov) from ".$this->esquema."boveda_mov where idmov='".$bov['idmov']."') > 0";
            $sqlUpdate[$nosql] = $sql;
            $nosql++;
...
//                    $response['message'] = "Error en ".$response['noquery']." ".$response['consultas']."!";
  //                  break;
                    if ($response['noquery'] >= 15) {
                        $response['message'] = "Existen movimientos por aplicar ó sin movimientos, verifique su Notas de crédito y/o Cierre de caja!";
                        break;
                    }
                }
            }
